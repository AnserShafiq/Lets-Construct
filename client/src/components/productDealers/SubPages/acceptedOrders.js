...
const AcceptedOrders = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`/productorders/acceptedorders/${userdata?.userName}`);
  ...
  const handleOrderDone = async (productId) => {
    try {
        await axios.put(`/productorders/${productId}`, { orderStatus: "Delivered" });
          ...
          {outputProducts.map(product => (
            <li key={product._id}>
              ...
              <div>
                ...
                <span className='listHead'>Address:</span> {product.deliveryAddress}<br/>
                <span className='listHead'>Product:</span> {product.orderedProduct}<br/>
                <span className='listHead'>Quantity:</span> {product.orderQuantity}<br/>
